...
- (void)start {
    [self execute:^{
        ...
        [t.delimitState addStartMarker:@"#"
                             endMarker:nil
                   allowedCharacterSet:[[NSCharacterSet newlineCharacterSet] invertedSet]];
...
- (void)__property {
    ...
    if ([self predicts:OBJCPARSER_TOKEN_KIND_MIS_PROPERTY, OBJCPARSER_TOKEN_KIND_MIS_MANY_TO_MANY,
                       OBJCPARSER_TOKEN_KIND_MIS_ONE_TO_MANY, OBJCPARSER_TOKEN_KIND_MIS_ONE_TO_ONE, 0]) {
        [self relationship_];
    }
    [self semi_];
...
- (void)__propertySpec {
    ...
    do {
        if ([self predicts:OBJCPARSER_TOKEN_KIND_ATOMICITY, 0]) {
            [self atomicity_];
        } else if ([self predicts:OBJCPARSER_TOKEN_KIND_ASSIGN, OBJCPARSER_TOKEN_KIND_COPY,
                                  OBJCPARSER_TOKEN_KIND_RETAIN, OBJCPARSER_TOKEN_KIND_STRONG,
                                  OBJCPARSER_TOKEN_KIND_WEAK, 0]) {
            [self strength_];
